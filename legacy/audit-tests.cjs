...
source+='\n globalThis.api={unlockTimeFromPredicate,addExact,piUnits,piDecimal,migrationRecipient,verifyTopWallets,ledgerOperations,classifyRecentEvents,record,addRecentOperation,saveCheckpoint,loadState,restoreFromD1,seedD1,syncD1Wallets,buildReport,getState:()=>state,setState:x=>state=x,setLoaded:x=>checkpointLoaded=x};';
const calls=[];
let respond=()=>({});
...
(async()=>{
 ...
 const origin='2026-01-01T00:00:00Z',start=Date.parse(origin),after=n=>({not:{rel_before:String(n)}});
 const unlock=p=>api.unlockTimeFromPredicate(p,origin);
 assert.equal(unlock({unconditional:true}),start);
 assert.equal(unlock(after(60)),start+60000);
 assert.equal(unlock({and:[after(60),after(90)]}),start+90000);
 assert.equal(unlock({or:[after(60),after(90)]}),start+60000);
 assert.equal(unlock({or:[after(60),{unconditional:true}]}),start);
 assert.equal(unlock({and:[after(60),{unsupported:true}]}),null,'unknown AND branch must not be discarded');
 assert.equal(unlock({or:[after(60),{unsupported:true}]}),null,'unknown OR branch must not be discarded');
 assert.equal(unlock({and:[after(60),{rel_before:'90'}]}),null,'bounded claim window has no permanent unlock date');
 assert.equal(unlock({not:{unconditional:true}}),null,'never claimable');
 assert.equal(unlock({not:{not:after(60)}}),start+60000);
 assert.equal(unlock({rel_before:'garbage'}),null);
 const exact={};api.addExact(exact,'total','9007199254.7409920');api.addExact(exact,'total','0.0000001');
 assert.equal(exact.totalExact,'9007199254.7409921','exact integer units beyond Number safe range');
 const decimal={};api.addExact(decimal,'total','0.1');api.addExact(decimal,'total','0.2');assert.equal(decimal.totalExact,'0.3000000');
 assert.throws(()=>api.piUnits('1.00000001'),/Invalid/);
 const ambiguous={type:'create_claimable_balance',source_account:state.wallet,claimants:[{destination:'A'},{destination:'B'}],transaction_hash:'ambiguous',amount:'100'};
 assert.equal(api.migrationRecipient(ambiguous),null);const quarantined={};api.addRecentOperation(quarantined,ambiguous);assert.equal(Object.keys(quarantined).length,0);assert.equal(state.recentAmbiguities.count,1);
